...
CanvasFrame.prototype.transform = function() {
    ...
    var i = l = x = y = 0, w = CANVAS_WIDTH, h = CANVAS_HEIGHT,
        j, xr, yr, k,
        diff, diffnew, dx, dy, xk, yk, jr, jl, r, besthit, step = demo.SCAN_STEP, parallax = demo.SCAN_PARALLAX, radius = demo.SCAN_RADIUS, abs = Math.abs;
    ...
    for (i = 0; i < len; i += 4) {
        ...
        if (x < CANVAS_WIDTH/2) {
            ...
            if (!(x % demo.GRID_FACTOR) && !(y % demo.GRID_FACTOR) && Math.random() > demo.STOCASTIC_RATIO) {
                ...
                for ( j = parallax; j >= 0; j-=step ) {
                    diffnew = 0;
                    for ( r = 0; r < radius; r++) {
                        dx = parseInt(Math.random()*2*r - r, 10);
                        dy = parseInt(Math.random()*2*r - r, 10);
                        if (x+dx > w || x+dx < 0 || yr+dy > h || yr+dy < 0) {
                            r--;
                            continue;
                        }
                        jr = ((yr + dy)*w - j + xr + dx)*4;
                        jl = ((y + dy)*w + x + dx)*4;
                        diffnew += Math.pow(
                            (videopx[jr] - videopx[jl])*(videopx[jr] - videopx[jl])
                            + (videopx[jr + 1] - videopx[jl + 1])*(videopx[jr + 1] - videopx[jl + 1])
                            + (videopx[jr + 2] - videopx[jl + 2])*(videopx[jr + 2] - videopx[jl + 2])
                        , 2);
                    }
